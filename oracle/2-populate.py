...
number_of_pictures = 10000 # large number, so data will end earlier


def get_rnd_date(begin, end):
	return "TO_DATE('{0}/{1}/{2}', 'dd/mm/yy')".format(randrange(1, 28), randrange(1, 12), randrange(begin,end))
	...
	with open('catalog-utf.csv') as f, \
		...
		for row in reader:
			...
			if pictures_i % 1000 == 0:
				picture_out.write('/\n')
				pools_pictures_out.write('/\n')
				tags_pictures_out.write('/\n')
