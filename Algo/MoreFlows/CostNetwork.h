...
public:
    ...
    public:
        ...
        NetworkEdgeIterator(CostNetwork& graph_, vtype v_, bool first) {
            graph = &graph_;
            if (first) {
                v = v_;
        ...
        NetworkEdgeIterator(CostNetwork& graph_, etype num) {
            graph = &graph_;
            e = num;
            edge = &graph->edges[e];
        }
        ...
        inline CostType getCost() { return edge->cost; }
    ...
    private:
        ...
        friend class CostNetwork;
    };

    CostNetwork() = default;

    CostNetwork(vtype n, vtype s, vtype t) {
        init(n, s, t);
    }
    ...
    inline void insertDirectedEdge(vtype u, vtype v, FlowType c, CostType cost, EST value) {
        insertEdgeLocal(u, v, c, 0, cost, value);
        insertEdgeLocal(v, u, 0, 0, -cost, value);
    }
    inline void insertDirectedEdge(vtype u, vtype v, FlowType c, CostType cost) {
        insertEdgeLocal(u, v, c, 0, cost, 0);
        insertEdgeLocal(v, u, 0, 0, -cost, 0);
    }
...
private:
    ...
    struct Edge {
        ...
        etype next = NullEdge;
        EST value;
        Edge(vtype u, vtype v, FlowType c, FlowType f, CostType cost_, EST value_)
                : from(u), to(v), capacity(c), flow(f), cost(cost_), value(value_) {}
    };
    ...
    void insertEdgeLocal(vtype from, vtype to, FlowType capacity, FlowType flow, CostType cost, EST value) {
        edges.push_back(Edge(from, to, capacity, flow, cost, value));
