...
/*void XMLParser::parse( std::string& s, CNode* parent )
{
	...
	is >> str;
	while( str[0] != '<' && is.good() ) {
	//parent->text.append( str );
	is >> str;
	}
	if( str[0] == '<' ) {
	// Добавляем главный тег.
	parent->tag = str.substr( 1, str.length() - 2 );
	std::cout << "Opentag " << parent->tag << std::endl;
	++mainCount;
	///////////////////}
		...
		if( str[0] != '<' && mainCount != 0 ) { // Текст, находящийся под областью действия тега.
			...
			if( tmp[0] != '/' ) { // Любой открывающийся.
				...
				if( mainCount == 0 ) { // Главный тег.
					parent->tag = tmp;
					++mainCount;
				} else if( tmp == parent->tag ) { // Такой же, открывающийся.
					++mainCount;
					parent->text.append( str + '\n' );
				} else { // Другой, открывающийся.
					++allCount;
				...
				std::cout << "Closetag " << tmp << std::endl;
				if( tmp == '/' + parent->tag ) { // Такой же, закрывающийся.
					--mainCount;
			...
			if( mainCount == 0 ) { // Главный тег закрылся и внутри был хотя бы один другой тег.
				parent->childs.push_back( new CNode );
				parse( parent->text, parent->childs.back() );
			}
		}
	}
}*/